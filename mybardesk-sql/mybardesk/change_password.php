    ...
    <div class="wrapper">
        <form action="" method="post">
            ...
            <div class="input-box">
                ...
                <input type="password" name="passwold" placeholder="old password" required>
                <i class='bx bx-lock-alt'></i>
            </div>
            <div class="input-box">

                <input type="password" name="passw" placeholder="new password" required>
                <i class='bx bx-lock-alt'></i>
            </div>
            <div class="input-box">

                <input type="password" name="passwrp" placeholder="new password again" required>
                <i class='bx bx-lock-alt'></i>
            </div>
...
<?php
if (isset($_POST['save'])) {
    $passwordold = $_POST['passwold'];
    $password = $_POST['passw'];
    $passwordrp = $_POST['passwrp'];
    ...
    foreach ($list as $value) {
        if ($_SESSION["username"] == $value['username'] && $passwordold == $value['password']) {
            if ($password==$passwordrp) {
                $add = $x->exec("UPDATE `kullanici` SET `password`='$password' WHERE `username`='$username'");
                header('location: profile.php');
            } else {
                echo '<script>alert("password verification is wrong");</script>';
